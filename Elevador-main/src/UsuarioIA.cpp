...
UsuarioIA::UsuarioIA(std::string nome, Porta* porta, SensorPresenca *sp):
...
{   
    ...
    while(true)
    {
        ...
        if(_ptrSensorP->numPessoasDentro()  >= maxNumPessoas)continue;   
        ...
        while(!_ptrPorta->abertaNoAndar(_andarDestinoUsuario) && 
            _ptrSensorP->numPessoasDentro()  <= maxNumPessoas)
        {
            //botao emergencia
        }
